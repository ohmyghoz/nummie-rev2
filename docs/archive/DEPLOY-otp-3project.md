# ⚠️ ARSIP — sudah TIDAK BERLAKU. Penggantinya: [`../DEPLOY.md`](../DEPLOY.md)

> Berkas ini menggambarkan dunia yang dibatalkan repo ini: **tiga project Vercel** (AGENTS.md §5
> menjadikannya satu) dan **auth ortu lewat OTP** ([ADR-0022](../decisions/0022-auth-ortu-otp-tanpa-password.md),
> dibatalkan [ADR-0023](../decisions/0023-auth-ortu-email-password.md)). URL dan bukti di dalamnya
> milik repo lama `Nummie-test`.
>
> **Disimpan, bukan dihapus** — jebakan yang ditemukannya nyata dan sebagiannya masih berlaku
> (env `NEXT_PUBLIC_*` wajib ada sebelum build pertama, Deployment Protection wajib mati untuk uji,
> alias `@copy` butuh berkas di atas root directory, SMTP sendiri wajib). Semuanya sudah dibawa ke
> `DEPLOY.md` yang baru. Yang tersisa di sini adalah riwayat: kenapa bentuk lama pernah masuk akal.

---

# Deploy — PWA di Vercel *(arsip 30 Juli 2026)*
