/* GENERATED oleh tools/unpack-mockups.mjs dari reference/mockups/parent-mobile.html — JANGAN DIEDIT.
...
class Component extends DCLogic {
  constructor(props){
    ...
    this.state = { authed:null, login:'pick', lgEmail:'', lgPass:'', kEmail:'', pin:'',
      tab:'dash', kid:'arthur', seg:'ok', insRange:'30d', insAll:false, insTipTab:'celebrate', insFrom:'2026-06-16', insTo:'2026-07-16', push:null, sheet:false, toast:null };
    this.initData();
  }
  initData(){
    const D = this.D = {};
  ...
  chip(K,sel,pick){
    ...
    return { pick, emo:K.emo, name:K.name, tier:K.tier,
      bg:on?'#6C4CE0':'#fff', bd:on?'#6C4CE0':'#E7E2F3', fg:on?'#fff':'#1E1A38',
      subFg:on?'rgba(255,255,255,.72)':'#7A7596', eBg:on?'rgba(255,255,255,.2)':'#F7F5FD' };
  }
  ...
  renderVals(){
    ...
    const nav=t=>S.tab===t?'#6C4CE0':'#7A7596';
    V.navDash=nav('dash'); V.navReq=nav('req'); V.navMis=nav('mis'); V.navIns=nav('ins'); V.navSet=nav('set');
    ...
    V.openSheet=()=>this.setState({sheet:true}); V.closeSheet=()=>this.setState({sheet:false});
    V.openAddKid=()=>this.setState({sheet:false,push:{type:'addkid',name:'',m:'',y:'',tier:null,touched:false,pin:''}});
    V.openSend=()=>this.setState({sheet:false,push:{type:'send',kid:S.kid,amt:'',src:'Gift',note:''}});
    V.openTake=()=>this.setState({sheet:false,push:{type:'take',kid:S.kid,wal:null,amt:'',why:''}});
    V.shSend=V.openSend; V.shTake=V.openTake; V.shAddKid=V.openAddKid;
    // requests
    ...
    V.segBtns=segDefs.map(s=>{ const on=S.seg===s.id, cnt=s.id==='ok'?cntOk:s.id==='todo'?cntTodo:0;
      return { pick:()=>this.setState({seg:s.id}), label:s.label, bg:on?'#fff':'transparent', fg:on?'#1E1A38':'#7A7596', sh:on?'0 1px 3px rgba(30,26,56,.1)':'none', cntShow:cnt>0, cnt, cntBg:s.id==='ok'?'#B26A00':'#6C4CE0' }; });
    ...
    V.reqItems=items.map(r=>{ const RK=D.kids[r.kid];
      ...
      const clickable=r.state==='done'||r.state==='declined';
      return { icon:this.icon(r.ic), title:r.title, amt:r.amt, sub:r.sub, kemo:RK.emo, kname:RK.name, when:r.when,
        ...
        showDecider:clickable&&!!r.decidedBy, deciderTxt:r.decidedBy?('Diputuskan oleh '+r.decidedBy+' · '+(r.decidedWhen||r.when)):'',
        cardClick:clickable?(()=>this.setState({push:{type:'reqdetail',reqId:r.id}})):(()=>{}), cardCursor:clickable?'pointer':'default',
        showFoot:pending,
    ...
    // insight
    {
      ...
      // section 1: how money is arranged (allocation over time)
      ...
      // section 3: grow wallet performance
      ...
      // section 6: chores/missions completion
    ...
    V.pInvite=!!P&&P.type==='invite'; V.pRd=!!P&&P.type==='reqdetail';
    V.pushTitle=!P?'':P.type==='send'?'Send money':P.type==='take'?'Take money':P.type==='addkid'?'Add a child':P.type==='allow'?'Allowance':P.type==='inv'?'Investments':P.type==='split'?'Money rules':P.type==='job'?'New job':P.type==='prize'?'New prize':P.type==='invite'?'Undang pasangan':P.type==='reqdetail'?'Detail permintaan':P.type==='hist'?(P.kid==='__all'?'All transactions':'Transactions'):P.type==='wallet'?(D.kids[P.kid]||K).name+'’s wallets':'Your bank rates';
    V.pushFootShow=!!P&&['inv','hist','wallet','reqdetail'].indexOf(P.type)<0;
    if(P&&P.type==='invite'){
    ...
    if(P&&P.type==='reqdetail'){
      const RR=D.reqs.find(x=>x.id===P.reqId)||{};
      const RK=D.kids[RR.kid]||K;
      const pillMap={talk:{t:'Talk',bg:'#E4F1FD',fg:'#1B6E97',bd:'#C8E4F8'},ok:{t:'Waiting',bg:'#FFF1D6',fg:'#B26A00',bd:'#F5DFAE'},todo:{t:'To do',bg:'#EDE7FC',fg:'#6C4CE0',bd:'#D3CBF5'},done:{t:'Done',bg:'#E1F6EC',fg:'#1B7A4B',bd:'#BCE8D2'},declined:{t:'Declined',bg:'#FCE8EA',fg:'#D64550',bd:'#F5C9CE'}};
      const pill=pillMap[RR.state]||pillMap.ok;
      V.rdIcon=this.icon(RR.ic); V.rdTitle=RR.title; V.rdAmt=RR.amt; V.rdSub=RR.sub;
      V.rdKemo=RK.emo; V.rdKname=RK.name;
      V.rdPillTxt=pill.t; V.rdPillBg=pill.bg; V.rdPillFg=pill.fg; V.rdPillBd=pill.bd;
      V.rdWhyShow=!!RR.why; V.rdWhy=RR.why||'';
      V.rdStoryShow=RR.type==='give'&&!!RR.story; V.rdStory=RR.story||'';
      V.rdDeciderShow=!!RR.decidedBy;
      V.rdDecider='Diputuskan oleh '+(RR.decidedBy||'')+' · '+(RR.decidedWhen||RR.when||'');
    }
    if(P&&P.type==='hist'){
